...
C = {
    'fwd':      ( 90, 200,  90),   # soft green
    'bwd':      ( 90, 110, 210),   # muted blue
    'left':     (190, 190,  60),   # warm yellow
    'right':    ( 60, 190, 210),   # teal
    'cw':       (170,  90, 190),   # lavender
    'ccw':      ( 90, 190, 190),   # mint
    'strL':     (160,  90, 160),   # mauve
    'strR':     (100, 170, 100),   # sage
    'stop':     ( 70,  70, 200),   # muted red
    'idle':     (100, 110, 115),   # cool gray
    'rhand':    ( 80, 200, 130),   # seafoam
    'lhand':    (190, 110, 190),   # dusty purple
    'speed':    ( 90, 180,  90),   # soft green
    'warn':     ( 60, 170, 210),   # amber-blue
    'override': ( 60,  90, 190),
    'text':     (200, 205, 210),   # off-white
    'dim':      (100, 108, 115),   # dim gray
    'panel':    ( 22,  26,  30),   # near-black panel
}
...
# ── Minimal speed bar (left edge, slim) ──────────────────────────
...
# ── Compact status panel (top-right) ─────────────────────────────
def draw_status_panel(frame, state, w, h, override_active=False):
    pw, ph  = 240, 148
    sx, sy  = w - pw - 8, 8
    ...
    if override_active:
        gcol    = C['override']
        gesture = f"OVR: {gesture}"

    # gesture — slightly larger, colored
    ...
    if sc_state == 'holding':
        remain = max(0, SPEED_CENTER_HOLD - state.get('sc_held',0))
        cv2.putText(frame, f"SpeedCtr: hold {remain:.1f}s", (sx+8,y),
                    FONT, 0.34, C['warn'], 1)
    elif sc_state == 'active':
        cv2.putText(frame, "SpeedCtr: ACTIVE", (sx+8,y),
                    FONT, 0.34, C['speed'], 1)

    if override_active:
        cv2.rectangle(frame, (sx,sy), (sx+pw,sy+ph), C['override'], 1)
...
def draw_info_bar(frame, state, w, h):
    """Single-row info strip at the bottom of the frame."""
    bh = 26
    by = h - bh
...
class HandController(Node):
    ...
    def __init__(self):
        ...
        self.create_subscription(Twist, '/system/status_twist',
                                 lambda msg: None, 10)
        ...
        self.get_logger().info("   Publishes → /hand/cmd_vel")
    ...
    def loop(self):
        ...
        # right_active: มือขวา fist (Rotate CW) ใช้คนเดียวได้ หรือมือซ้ายไม่มี/open/speed/fist
        right_active    = (right_lms is not None and not r_open and
                           (left_lms is None or l_open or l_speed_gesture or l_fist))
        # left_active: มือซ้าย fist (Rotate CCW) ใช้คู่กับมือขวา fist ได้
        ...
        draw_status_panel(frame, self.state, w, h, self.override_active)
        draw_direction_arrow(frame, gesture, w, h)
        draw_info_bar(frame, self.state, w, h)

        # open-palm labels (minimal, one line)
        if r_open or l_open:
            tags = []
